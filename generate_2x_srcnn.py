...
# This file is used to generate a 2x super resolution image of the the given image using SRCNN-Chainer.
...
class Generate2X:
    """split the given image into 64*64*3 images and generate 2x super resolution 128*128*3 images by srcnn,
    ...
    def img2bic(self, image):
        """return bicubic image"""
        image_bic = cv2.resize(image,
                               (2*self.cropsize, 2*self.cropsize),
                               interpolation=cv2.INTER_CUBIC)
        return image_bic

    def gensuper(self, image):
        """low resolution(bic) --> super resolution"""
...
def generate_2x(image_path, gen_data):
    gen = Generator()
    super = Generate2X(image_path, gen, gen_data).processing()
    save_path, _ = image_path.split(".")
    cv2.imwrite(save_path + "_super_2x.png", super)


def main():
    parser = argparse.ArgumentParser(description='SRCNN-Chainer by Chainer')
    parser.add_argument('--input_file', '-i', type=str, default="butterfly.png",
                        help='the input file to SR')
    parser.add_argument('--gen_data', '-gd', type=str, default="gen_iter_4000.npz",
